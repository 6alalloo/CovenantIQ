...
class AlertLifecycleIntegrationTest {
    ...
    @Test
    void alertLifecycleOpenToAcknowledgedToUnderReviewToResolved() throws Exception {
        String analystToken = accessTokenFor("analyst@demo.com", "Demo123!");
        String riskLeadToken = accessTokenFor("risklead@demo.com", "Demo123!");

        long loanId = createLoan(analystToken, "Lifecycle Borrower");
        ...
        mockMvc.perform(patch("/api/v1/alerts/{alertId}/status", alertId)
                        ...
                        .content("""
                                {
                                  "status": "RESOLVED",
                                  "resolutionNotes": "Issue reviewed and accepted."
                                }
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("RESOLVED"))
                .andExpect(jsonPath("$.resolutionNotes").value("Issue reviewed and accepted."));
    }
